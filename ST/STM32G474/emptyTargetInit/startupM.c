...
void ResetISR(void)
{
    SCB_CPACR |= (3 << 20) | (3 << 22); // set CP10 and CP11 Full Access
    SCB_VTOR = 0x08000000;              // Vector Table Relocation in Internal SRAM
...
{
    TIMER2_SR &= ~(1 << CC1IF); // clear interrupt
    InterruptRoutine();     
    TIMER2_CR1 |= (1 << CEN); //enable interrupt
}
