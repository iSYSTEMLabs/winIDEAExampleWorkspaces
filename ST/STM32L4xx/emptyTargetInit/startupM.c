...
void (* const g_pfnVectors[])(void) =
{
    ...
    TimerInterruptHandler,      //TIM1_BRK 0x0000_00A0
...
{     
    ...
    if(CHECK_BIT(PWR_CSR,8))
    {                                
      //enter here if reset was caused by a low power mode
    }
...
{  
  ...
  if ( dwLR & 4 )  
    ...
    asm(" mrs r0, msp");  
  if(CHECK_BIT(PWR_CSR,0))
  {                                
    //enter here if interrupt was caused by the low power mode
  }
  while ( 1 );  
}  
