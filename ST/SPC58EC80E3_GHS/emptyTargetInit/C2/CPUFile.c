...
void MC_INIT(void)
{
  ...
  // XOSC = 40MHZ
  /* Configure PLL0 Dividers - PHI:120MHz PHI1:120MHz from 40Mhx XOSC - VCO:800MHz */
  PLLDIG.PLL0DV.B.RFDPHI1 = 1;
  PLLDIG.PLL0DV.B.RFDPHI = 1;
  PLLDIG.PLL0DV.B.PREDIV = 2;
   
  PLLDIG.PLL0DV.B.MFD = 6; //for XOSC
...
void MC_RUN_CORES(void)
{
   ...
   MC_ME.CADDR1.R = 0x01013001; // Set Start address for core 0: Will reset and start
   
   // Enable HSM core if it is not enabled via DCF record
   if (SSCM.UOPS.B.HSE == 0)
   {
     MC_ME.CCTL4.R = 0x00FE;       // Enable HSM core in all modes
...
void targetEnableInterrupts()
{
  ...
  INTC_PSR226 &= ~(1 << INTC_PSR226_PRC_SEL0);
  INTC_PSR226 |= (1 << INTC_PSR226_PRC_SEL2);
  INTC_PSR226 |= (1 << INTC_PSR226_PRI);
  INTC_CPR2 &= ~(0x3F << INTC_CPR2_PRI);
