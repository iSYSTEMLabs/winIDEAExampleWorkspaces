...
//   base address
//  e_lwi	r3, 0xFC030000
  lis r3,0xFC03  // base address of the L2SRAM, 64-bit word aligned
  ori r3,r3,0  // not needed for this address but could be forothers
   
//   FLASH_PFCR1
//  e_lwi	r4, 0xFFFF4457
    lis r4,0xFFFF  // base address of the L2SRAM, 64-bit word aligned
    ori r4,r4,0x4457  // not needed for this address but could be forothers
  
//   FLASH_PFCR2
//  e_lwi	r5, 0xFFFF0057
    lis r5,0xFFFF  // base address of the L2SRAM, 64-bit word aligned
    ori r5,r5,0x0057  // not needed for this address but could be forothers

//  FLASH_PFCR3
//  e_lwi	r6, 0x00008000
//    lis r6,0x0000  // base address of the L2SRAM, 64-bit word aligned
    li  r6,0x0000
    ori r6,r6,0x8000  // not needed for this address but could be forothers
...
init_RAM:
   lis r11,__RAM_START_ADDR@ha
   ori r11,r11,__RAM_START_ADDR@l 
   lis r12,__RAM_SIZE@ha  //get number of loop repetitions (each time writes 32GPRs * 4 bytes)
...
/*--------- Initialized sdata2 - ".sdata2" --------------------------------------  */
SDATA2COPY:
  lis     r9,       __ROM_SDATA2_SIZE@ha // Load upper SRAM load size
  addi    r9,   r9, __ROM_SDATA2_SIZE@l  // Load lower SRAM load size into R9
  cmpi    r9,   0                       // Compare to see if equal to 0
  beq     ENDCOPY2                       // Exit cfg_ROMCPY if size is zero
  mtctr   r9                            // Store no. of bytes to be moved in counter
  lis     r10, __ROM_SDATA2_ADDR@ha       // Load address of first SRAM load into R10
  addi    r10, r10, __ROM_SDATA2_ADDR@l  // Load lower address of SRAM load into R10
  subi    r10, r10, 1                   // Decrement address
  lis     r5,  __RAM_SDATA2_ADDR@ha       // Load upper SRAM address into R5
  addi    r5,  r5,  __RAM_SDATA2_ADDR@l  // Load lower SRAM address into R5
  subi    r5,  r5,  1                   // Decrement address
SDATA2CPYLOOP:
  lbzu    r4,  1(r10)                   // Load data byte at R10 into R4
  stbu    r4,  1(r5)                    // Store R4 data byte into SRAM at R5
  bdnz    SDATA2CPYLOOP                  // Branch if more bytes to load from ROM
ENDCOPY2:
