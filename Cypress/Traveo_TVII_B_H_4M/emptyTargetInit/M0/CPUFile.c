...
void initTimer()
{
  ...
  PERI_CLOCK_CTL_38 = 0x00000100;
  CLK_TIMER_CTL |= (0x1 << CLK_TIMER_CTL_ENABLE);

  PERI_PCLK_GR1_DIV_16_CTL0 |= (0x27 << 8);
  ...
  PERI_PCLK_GR1_DIV_CMD = tempDIV_CMD;
  
  TCPWM0_GRP0_CNT0_CTRL = 0;
  
  TCPWM0_GRP0_CNT0_CTRL |= (0x1 << 0);
  
  TCPWM0_GRP0_CNT0_CC0 = 200; // Set timer to 1ms (board rev. C), for older revisions 1ms = 200
  TCPWM0_GRP0_CNT0_CC0_BUFF = 200;
  
  TCPWM0_GRP0_CNT0_PERIOD = 200;
...
{
  CM0_SYSTEM_INT_CTL_351 = 0x0;
  CM0_SYSTEM_INT_CTL_351 |= (0x3 << CM0_SYSTEM_INT_CTL_351_INT_IDX);
  CM0_SYSTEM_INT_CTL_351 |= (0x1 << CM0_SYSTEM_INT_CTL_351_INT_VALID);
