...
#***************************************************************************/
 ...
 #            ************* 
    .section .text_vle 
    .globl  TimerInterruptHandler_2
    .global DebugInterruptBr
    
    .extern InterruptRoutine

    .equ  INTC_IACKR2, 0xFC040028  #  core 2 Interrupt Acknowledge Reg address
    .equ  INTC_EOIR2,  0xFC040038  #  core 2 End Of Interrupt Reg address
    
    .align 4
    
    .function "TimerInterruptHandler_2", TimerInterruptHandler_2, TimerInterruptHandler_END_2 - TimerInterruptHandler_2
    .size TimerInterruptHandler_2, TimerInterruptHandler_END_2 - TimerInterruptHandler_2 
    .type TimerInterruptHandler_2, @function

      
TimerInterruptHandler_2:
prolog_core_2:                     # PROLOGUE 
    ...
    se_stw   r3, 0x0C (r1)                                           
    e_lis    r3, INTC_IACKR2@ha    # Store address of IACKR in r3
    e_lwz    r3, INTC_IACKR2@l(r3) # Store contents of IACKR in r3 (this is vector table address)                                       
...
epilog_core_2:                     # EPILOGUE
    ...
    e_lwz    r11, 0x48 (r1)
    e_lwz    r12, 0x4C (r1)
    e_lis    r3, INTC_EOIR2@ha     # Load upper half of EIOR address to r3                                                                                                                                                 
    wrteei 0                       # Disable interrupts    
    mbar 0                         # Ensure interrupt flag has finished clearing
                                   # before writing to INTC_EIOR
    se_stw   r3, INTC_EOIR2@l(r3)  # Load lower half of EIOR address to r3 and
