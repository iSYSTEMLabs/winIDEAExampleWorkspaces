...
#***************************************************************************/
 ...
 #            ************* 
    .section .text_vle 
    .globl  TimerInterruptHandler_1
    .global DebugInterruptBr
    
    .extern InterruptRoutine

    .equ  INTC_IACKR1, 0xFC040024  #  core 1 Interrupt Acknowledge Reg address
    .equ  INTC_EOIR1,  0xFC040034  #  core 1 End Of Interrupt Reg address
    
    .align 4
    
    .function "TimerInterruptHandler_1", TimerInterruptHandler_1, TimerInterruptHandler_END_1 - TimerInterruptHandler_1
    .size TimerInterruptHandler_1, TimerInterruptHandler_END_1 - TimerInterruptHandler_1 
    .type TimerInterruptHandler_1, @function

      
TimerInterruptHandler_1:
prolog_core_1:                     # PROLOGUE 
    ...
    se_stw   r3, 0x0C (r1)                                           
    e_lis    r3, INTC_IACKR1@ha    # Store address of IACKR in r3
    e_lwz    r3, INTC_IACKR1@l(r3) # Store contents of IACKR in r3 (this is vector table address)                                       
...
epilog_core_1:                     # EPILOGUE
    ...
    e_lwz    r11, 0x48 (r1)
    e_lwz    r12, 0x4C (r1)
    e_lis    r3, INTC_EOIR1@ha     # Load upper half of EIOR address to r3                                                                                                                                                 
    wrteei 0                       # Disable interrupts    
    mbar 0                         # Ensure interrupt flag has finished clearing
                                   # before writing to INTC_EIOR
    se_stw   r3, INTC_EOIR1@l(r3)  # Load lower half of EIOR address to r3 and
