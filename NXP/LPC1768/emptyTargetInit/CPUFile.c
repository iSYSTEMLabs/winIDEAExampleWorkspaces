...
int LPC1766_InitClock()
{
  ...
  SCS |= (1 << 5);               // enable main oscillator - 12 MHz
  CLKSRC |= (1 << 0);            // select main oscillator as source for PLL0

  PLL0CFG |= (149 << 0);          // M = 150
  PLL0CFG |= (11 << 16);          // N = 12
  //MEMWR32(PLL0CFG,35);          // 60MHz clock 
  ...
  /*
    SWD trace only works on 60 MHz clock, that is why clock is not
    initialized to 100 MHz.
  */
  MEMWR32(CCLKCFG, 0x4);          // CPU Clock divider is 5 (60MHz)
  //MEMWR32(CCLKCFG, 0x3);          // CPU Clock divider is 4 (75MHz)
