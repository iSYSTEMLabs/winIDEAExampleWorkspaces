...
void triggerHardwareProcess()
{
  ...
  while(MC_ME.CTL_KEY.R != 0x0000A50F);
  MC_ME.CTL_KEY.R = 0x0000A50F;
  while(MC_ME.CTL_KEY.R != 0x00005AF0);
}
...
void start_RTU0_R52_2()
{
  /*Hack delay to wait for RTU1_R52_0 to disable lockstep.
   A proper app should do this by setting a shared variable.*/
  int i;
  for(i = 0; i < 100000; i++);
  
  // Entry point
  MC_ME.PRTN1_CORE2_ADDR.R = (uint32_t)RTU0_R52_2_START_ADDRESS;
  
  // Release the reset of core from MC_RGM
  MC_RGM.PRST1_0.B.PERIPH_67_RST = 0;
  
  if(MC_ME.PRTN1_CORE2_STAT.B.CCS != 1)
  {
    // Enable clock to core from MC_ME
    MC_ME.PRTN1_CORE2_PCONF.R |= 1;
    MC_ME.PRTN1_CORE2_PUPD.R |= 1;
    
    triggerHardwareProcess();
    
    // Wait for core clock to get enabled
    while(MC_ME.PRTN1_CORE2_STAT.B.CCS != 1);
  }
  
  // Wait for MC_RGM to release reset for core
  while(MC_RGM.PSTAT1_0.B.PERIPH_67_STAT != 0);
}

void start_RTU0_R52_3()
{
  // Entry point
  MC_ME.PRTN1_CORE3_ADDR.R = (uint32_t)RTU0_R52_3_START_ADDRESS;
  
  // Release the reset of core from MC_RGM
  MC_RGM.PRST1_0.B.PERIPH_68_RST = 0;
  
  if(MC_ME.PRTN1_CORE3_STAT.B.CCS != 1)
  {
    // Enable clock to core from MC_ME
    MC_ME.PRTN1_CORE3_PCONF.R |= 1;
    MC_ME.PRTN1_CORE3_PUPD.R |= 1;
    
    triggerHardwareProcess();
    // Wait for core clock to get enabled
    while(MC_ME.PRTN1_CORE3_STAT.B.CCS != 1);
  }
  
  // Wait for MC_RGM to release reset for core
  while(MC_RGM.PSTAT1_0.B.PERIPH_68_STAT != 0);
}
...
void start_RTU1_R52_2()
{
  // Entry point
  MC_ME.PRTN2_CORE2_ADDR.R = (uint32_t)RTU1_R52_2_START_ADDRESS;
  
  // Release the reset of core from MC_RGM
  MC_RGM.PRST2_0.B.PERIPH_131_RST = 0;
  
  if(MC_ME.PRTN2_CORE2_STAT.B.CCS != 1)
  {
    // Enable clock to core from MC_ME
    MC_ME.PRTN2_CORE2_PCONF.R |= 1;
    MC_ME.PRTN2_CORE2_PUPD.R |= 1;
    
    triggerHardwareProcess();
    
    // Wait for core clock to get enabled
    while(MC_ME.PRTN2_CORE2_STAT.B.CCS != 1);
  }
  
  // Wait for MC_RGM to release reset for core
  while(MC_RGM.PSTAT2_0.B.PERIPH_131_STAT != 0);
}

void start_RTU1_R52_3()
{
  // Entry point
  MC_ME.PRTN2_CORE3_ADDR.R = (uint32_t)RTU1_R52_3_START_ADDRESS;
  
  // Release the reset of core from MC_RGM
  MC_RGM.PRST2_0.B.PERIPH_132_RST = 0;
  
  if(MC_ME.PRTN2_CORE3_STAT.B.CCS != 1)
  {
    // Enable clock to core from MC_ME
    MC_ME.PRTN2_CORE3_PCONF.R |= 1;
    MC_ME.PRTN2_CORE3_PUPD.R |= 1;
    
    triggerHardwareProcess();
    
    // Wait for core clock to get enabled
    while(MC_ME.PRTN2_CORE3_STAT.B.CCS != 1);
  }
  
  // Wait for MC_RGM to release reset for core
  while(MC_RGM.PSTAT2_0.B.PERIPH_132_STAT != 0);
}

void start_SMU_M33()
{
  
  MDM_AP.CONTROL.B.SMU_EDBGREQ = 0x1;
  // Entry point
  MC_ME.PRTN0_CORE0_ADDR.R = (uint32_t)SMU_M33_START_ADDRESS;
  
  // Release the reset of core from MC_RGM
  MC_RGM.PRST0_0.R = 0x00;
  
  if(MC_ME.PRTN0_CORE0_STAT.B.CCS != 1)
  {
    // Enable clock to core from MC_ME
    MC_ME.PRTN0_CORE0_PCONF.R |= 1;
    MC_ME.PRTN0_CORE0_PUPD.R |= 1;

    triggerHardwareProcess();

    while(MC_ME.PRTN0_CORE0_STAT.B.CCS != 1);
  }
  
  // Wait for MC_RGM to release reset for core
  while(MC_RGM.PRST0_0.R != 0);
}

void R52LockStepConfiguration()
{
  /*Disable lockstep only for R52 Cluster 1. The R52 core can't 
  configure lockstep for itself. Cluster 0 lockstep config is done on the RTU1_R52_0 core.*/
  RTU1__GPR.CFG_CORE.R |= 1;
}
