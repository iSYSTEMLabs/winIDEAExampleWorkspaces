 ...
 * decode.
 */
#define  CONFIG_STM_TIMESTAMP_FREQUENCY  0L
#define  CONFIG_STM_SYNC_FREQUENCY       0
 ...
 * for us.
 */
#define  CONFIG_STM_USE_COMPRESSION  0
#define  CONFIG_STM_USE_TRACEID      5
...
typedef volatile unsigned long long  STM_STIM; //64bit
...
struct stmPort {
    ...
    STM_STIM    G_FLAGTS; // Flag with timestamp
    STM_STIM    G_FLAG;   // Flag, may be used to indicate end of message.
    STM_STIM    G_TRIGTS; // Trigger with timestamp
    STM_STIM    G_TRIG;   // Trigger
...
#define STM64(a, p, type) *((volatile unsigned long long*) &((a)->port[p].type))
