...
{ 
  ...
  if (GpioOffOn)
  { 
    GIODSET = 0xFF; // set port A (pin A5 on Hercules board)
  }
  else
  {
    GIODCLR = 0xFF; // clear port A (pin A5 on Hercules board)
  } 
}

  //rt TimerInterrupt: profilerFunctionName=ProfilerC_Interrupt, timeoutMSec=1.000, maxDeltaPeriodPct=5
  //rt DaqTimerInterrupt: daqItem=daqInterruptCounter, periodInMicroSeconds=49000, maxDeltaPeriodInPercents=4
